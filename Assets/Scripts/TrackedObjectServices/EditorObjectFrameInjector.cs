...
public class EditorObjectFrameInjector : MonoBehaviour
{
    ...
    {
#if UNITY_EDITOR
        //Debug.Log("SessionState enabled viziualations = " + SessionState.enableGenericVisualizations.Value);
        if(SessionState.enableGenericVisualizations.Value)
        {
            ...
            foreach (var det in Detections)
            {
                if (!HelperInstances.ContainsKey(det))
                {
                    ...
                    offset -= 10f; //changed offset from -0.1f to -10f
                }
            ...
            foreach (var helperInstance in HelperInstances)
            {
                ...
                if (helperInstance.Key.Enabled)
                {
                    ...
                    if (detection == null)
                    {
                        ...
                        Debug.Log("Added new detection: " + detection.label);
                    }
