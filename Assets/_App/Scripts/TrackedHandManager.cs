...
{
    /// <summary>
    /// Initializes prefabs for the hands and optionally the fingertips
    /// Updates the position and orientation of the generated prefabs
    /// </summary>
    public class TrackedHandManager : MonoBehaviour
    {
        ...
        private void OnDestroy()
        {
            ...
            {
                Destroy(m_SpawnedLeftHand);
            }
            if (m_SpawnedRightHand)
            {
                Destroy(m_SpawnedRightHand);
            }
            if (m_SpawnedLeftHandTip)
            {
                Destroy(m_SpawnedLeftHandTip);
            }
            if (m_SpawnedRightHandTip)
            {
                Destroy(m_SpawnedRightHandTip);
            }
        }
