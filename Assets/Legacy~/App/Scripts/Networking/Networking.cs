...
/// See Networking.Chess for IChessControl implementation
[System.Serializable]
public partial class Networking : MonoBehaviour
{
    ...
    private int _targetPacketVersionNumber = 13;
    ...
    public enum packet_type
    {
        ...
        // Chess status
        packet_server_chessboard = 50,                      // v6 Handled
        packet_server_chess_state = 51,                     // v6 Handled
        packet_server_chess_legal_moves = 52,               // v6 Handled
        packet_server_chess_suggested_move = 53,		    // v6 Handled, but not used for visualization yet
        packet_server_chess_piece_detector_moves = 54,      // v6 Handled
        ...
        // Chess commands
        packet_client_chess_start_new_game = 900,           // v6 Implemented, can be used with help of chesscontroller
        packet_client_chess_play_move = 901,                // v6 Implemented, but needs string innpu
        packet_client_chess_request_detect_board = 902,     // v6 Implemented, but not invoked yet
        packet_client_chess_request_legal_moves = 903,      // v6 Automatically called after each chessstate update
        packet_client_chess_request_suggested_move = 904,   // v6 Implemented, but not invoked yet
        packet_client_chess_request_compare_boards = 905	// v6 Implemented
    };
    ...
    private void OnReceivedPacket(byte packetType, object[] data)
    {
        ...
        switch (packetType)
        {
            ...
            case (byte)packet_type.packet_server_video_replay_stopped:
                break;
            case (byte)packet_type.packet_server_chessboard:
                UpdatedChessboard((ChessBoard)data);
                break;
            case (byte)packet_type.packet_server_chess_state:
                UpdatedChessState((ChessState)data);
                break;
            case (byte)packet_type.packet_server_chess_legal_moves:
                UpdatedLegalMoves((LegalMoves)data);
                break;
            case (byte)packet_type.packet_server_chess_suggested_move:
                UpdatedSuggestedMove((SuggestedMove)data);
                break;
            case (byte)packet_type.packet_server_chess_piece_detector_moves:
                Debug.Log("Recieving piece detector moves");
                //UpdatedDetectorMoves((DetectorMoves)data);
                break;
        ...
        _listenThread = new Thread(new ThreadStart(delegate
        {
            while (_isClientMode && _isListening)
            {
                ...
                try
                {
                    ...
                    var data = MsgPack.Deserialize<object[]>(stream);
    ...
    private void PingServer(packet_type packetType = packet_type.packet_client_ping)
    {
        ...
        else
        {
            ...
            MsgPack.Serialize(msg, outputStream);
    ...
    private void Awake()
    {
        ...
        ServiceRegistry.RegisterService<IChessControl>(this);
    }
