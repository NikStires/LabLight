        ...
        private void OnPhotoCaptured(PhotoCapture.PhotoCaptureResult result, PhotoCaptureFrame photoCaptureFrame)
        {
            if (result.success)
            {
                ...
                if (photoCaptureFrame.hasLocationData)
                {
                    ...
                    var axis = GameObject.Instantiate(axisPrefab);

                    //Matrix4x4.TRS

                    var planes = projectionMatrix.decomposeProjection;

                    //axis.transform = cameraToWorldMatrix
                }
                else
                {
                    if (text != null)
