...
public class WellPlateViewController : ModelElementViewController
{
    ...
    private void enableHighlight(HighlightAction action)
    {
        ...
        foreach(string id in action.chainIDs)
        {
            ...
            toggleTransform(Markers, SessionState.ShowBB.Value, id, parsedColor);
                ...
                foreach(Transform child in infoPanel.GetComponentInChildren<Transform>())
                {
                    switch(child.name)
                    {
                        ...
                        case "InfoDisplay":
                            if(actions[0].actionName.Contains("transfer") && actions.Count() == 1)
                            {
                                child.GetComponent<TextMeshProUGUI>().text = "Transfer " + actions[0].volume.Item1 + actions[0].volume.Item2 + " " + (actions[0].isSource ? "from " : "into ") + ((ModelArDefinition)this.arDefinition).name;
                            }else
                            {
                                child.GetComponent<TextMeshProUGUI>().text = "Load " + actions[0].volume.Item1 + actions[0].volume.Item2 + " from ";
                            }
                            //enable source
                            child.GetComponent<TextMeshProUGUI>().text += "<" + colorHex + ">";
                            child.GetComponent<TextMeshProUGUI>().text += "test";
                            child.GetComponent<TextMeshProUGUI>().text += actions.Count() == 2 ? (actions[0].chainIDs[0] + (actions[0].chainIDs.Count() > 1 ? ("-" + actions[0].chainIDs[actions[0].chainIDs.Count()-1]) : "")) : (actions[0].actionName.Contains("transfer") ? "" : actions[0].contents.Item2);
                            child.GetComponent<TextMeshProUGUI>().text += "</color>";

                            if(actions.Count() == 2)
                            {
                                Color targetColor;
                                if(ColorUtility.TryParseHtmlString(actions[1].colorInfo.Item1, out targetColor))
                                {
                                    targetColor.a = 255;
                                    child.GetComponent<TextMeshProUGUI>().color = targetColor;
                                }
                                child.GetComponent<TextMeshProUGUI>().text += "<" + actions[1].colorInfo.Item1 + ">";
                                child.GetComponent<TextMeshProUGUI>().text += actions[0].actionName.Contains("transfer") ? (actions[1].chainIDs.Count() > 1 ? (actions[1].chainIDs[0] + "-" + actions[1].chainIDs[actions[1].chainIDs.Count()-1]) : actions[1].chainIDs[1]): "";
                                child.GetComponent<TextMeshProUGUI>().text += "</color>";
                            }else
                            {
                                child.GetComponent<TextMeshProUGUI>().text += "<" + colorHex + ">";
                                child.GetComponent<TextMeshProUGUI>().text += actions[0].actionName.Contains("transfer") ? "" : (actions[0].chainIDs.Count() > 1 ? (actions[0].chainIDs[0] + "-" + actions[0].chainIDs[actions[0].chainIDs.Count()-1]) : actions[0].chainIDs[0]);
                                child.GetComponent<TextMeshProUGUI>().text += "</color>";
                            }

                            if(actions.Count() == 1 && actions[0].actionName.Contains("transfer"))
                            {
                                child.GetComponent<TextMeshProUGUI>().text += "<" + colorHex + ">";
                                child.GetComponent<TextMeshProUGUI>().text += (actions[0].chainIDs.Count() > 1 ? (actions[0].chainIDs[0] + "-" + actions[0].chainIDs[actions[0].chainIDs.Count()-1]) : actions[0].chainIDs[0]);
                                child.GetComponent<TextMeshProUGUI>().text += "</color>";
                            }else
                            {
                                child.GetComponent<TextMeshProUGUI>().text += "into";
                            }
                        ...
                        case "InfoDisplay":
                            if(actions[0].actionName.Contains("transfer") && actions.Count() == 1)
                            {
                                ModelArDefinition temp = (ModelArDefinition)this.arDefinition;
                                child.GetComponent<TextMeshProUGUI>().text = "Transfer " + actions[0].volume.Item1 + " " + actions[0].volume.Item2 + " " + (actions[0].actionName.Contains("transfer") ? (actions[0].isSource ? "from " : "into " ) : "from ") + temp.name;
                            }else
                            {
                                child.GetComponent<TextMeshProUGUI>().text = "Load " + actions[0].volume.Item1 + " " + actions[0].volume.Item2 + " " + (actions[0].actionName.Contains("transfer") ? (actions[0].isSource ? "from " : "into " ) : "from ");
                            }
                            break;
                        case "SourceDisplay":
                            child.GetComponent<TextMeshProUGUI>().text =  actions.Count() == 2 ? (actions[0].chainIDs[0] + (actions[0].chainIDs.Count() > 1 ? ("-" + actions[0].chainIDs[actions[0].chainIDs.Count()-1]) : "")) : (actions[0].actionName.Contains("transfer") ? "" : actions[0].contents.Item2);//(actions[0].actionName.Contains("transfer") ? ((actions[0].isSource ? "Source: " : "Target: ") + (actions[0].chainIDs.Count() > 1 ? (actions[0].chainIDs[0] + "-" + actions[0].chainIDs[actions[0].chainIDs.Count()-1]): actions[0].chainIDs[0])) : actions[0].contents.Item2);
                        ...
                        case "TargetPrefix":
                            ...
                            {
                                child.GetComponent<TextMeshProUGUI>().text = "into";
                                child.GetComponent<TextMeshProUGUI>().color = new Color(255, 255, 255);
                            }
                            break;
                        case "TargetDisplay":
                            if(actions.Count() == 2)
        ...
        if(currActions != null)
        {
            foreach(HighlightAction action in currActions)
            {
                foreach(string id in action.chainIDs)
                {
                    ...
                    toggleTransform(Markers, (SessionState.ShowBB.Value && value), id);
                }
            }
        }
