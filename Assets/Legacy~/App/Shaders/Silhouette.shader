...
Shader "Valve/VR/Silhouette"
{
	...
	Properties
	{
		...
		g_vOutlineColorOutside("Outline Color", Color) = (.5, .5, .5, 1)
...
#include "UnityCG.cginc"
		...
		UNITY_DEFINE_INSTANCED_PROP(float4, g_vOutlineColorOutside)
	...
	PS_INPUT Extrude(PS_INPUT vertex)
	{
		...
		extruded.vPositionPs.xy += vOffsetPs.xy * UNITY_ACCESS_INSTANCED_PROP(Props, g_flOutlineWidth);
		extruded.vTexcoord = float2(1, 0);
	...
	fixed4 MainPs(PS_INPUT i) : SV_Target
	{
		...
		return UNITY_ACCESS_INSTANCED_PROP(Props, float4(g_vOutlineColor.rgb, i.vTexcoord.x));
	}
		...
		SubShader
	{
			...
			Pass
		{
			...
			//Blend SrcAlpha DstAlpha // Traditional transparency
			BlendOp Max
